...
export default function SessionsCards({ items, calendarUrl }: SessionsCardsProps) {
  ...
  // Check if Time flag exists in webPublishingStatus
  const hasTimeFlag = (session: Session & { webPublishingStatus?: string[] }) => {
    return session.webPublishingStatus?.includes("Time") ?? true; // default to true if no webPublishingStatus
  };
  ...
  return (
    <>
      ...
      {items.length === 0 ? (
        <Alert className="border-azure/20 bg-azure/5">
          <Info className="text-azure h-5 w-5" />
          <AlertTitle>Scheduling in Progress</AlertTitle>
          <AlertDescription>Your session is currently being scheduled. Check back soon for updates.</AlertDescription>
        </Alert>
      ) : (
